HotWheels 分布式定时任务管理系统
===========================

我们都知道linux自带的crontab简单的定时任务管理工具 

    1、只能支持分钟级别的定时粒度。
    2、如果定时任务增加，维护会越来越负复杂，成本越来越大。
    3、在分布式系统中，crontab分布在每台机器上，查询问题需要一台一台机器去找日志。
    4、crontab 存在单点问题，对于不能重复执行的定时任务很伤脑筋。
    5、公司有各个业务线，各个业务线都有定时任务的需求，

所以需要集中式的任务管理系统，来提高管理效率，快速定位问题。
它具有以下的功能：

    1、友好的web界面，方便使用这操作
    2、集中式的管理后台方便我们配置定时任务
    3、定时任务执行失败，能及时告警通知
    4、方便用户查询脚本执行情况
    5、支持秒级别的定时粒度
    6、可以设置并行支持脚本个数
